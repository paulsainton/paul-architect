          ...
          <>
            ...
            {TUNNEL_NAV.map((t) => {
              ...
              return (
                ...
                >
                  <t.icon className={`w-4 h-4 ${isActive ? t.color : ""}`} aria-hidden="true" />
                  <span className="flex-1">{t.label}</span>
                  {status === "active" && <ChevronRight className="w-3 h-3 text-accent animate-pulse" aria-hidden="true" />}
                  {status === "completed" && <CheckCircle className="w-3 h-3 text-status-success" aria-label="Étape terminée" />}
                  {status === "error" && <span className="w-2 h-2 rounded-full bg-status-error" aria-label="Erreur" />}
                </Link>
              );
            })}
          </>
